...
(function () {
  ...
  /* ---------- Tab switching ---------- */
  const renderers = { dashboard: renderDashboard, programs: renderPrograms, activities: renderActivities, team: renderTeam, gallery: renderGallery };
  ...
  function renderPrograms() {
    ...
    el.innerHTML = `
      ...
      <table class="admin-table">
        <thead><tr><th>Program</th><th>Slug</th><th>Status</th><th style="text-align:right;">Actions</th></tr></thead>
  ...
  function openProgramModal(program) {
    ...
    program = program || { icon: '🌿', title: '', slug: '', shortDesc: '', fullDesc: '', active: true };
    openModal(`
      <button class="modal-close" onclick="closeModal()">✕</button>
      <h3>${editing ? 'Edit Program' : 'Add Program'}</h3>
      <form id="programForm">
        <label>Icon (emoji)</label><input name="icon" value="${program.icon}" required />
        ...
        ${!editing ? '<p style="font-size:.75rem;color:rgba(22,34,28,.5);margin-top:10px;">Note: a brand-new program has no matching individual page in the static export yet — you\'d need to add one manually (copy an existing file under /programs/). Editing existing programs works fully.</p>' : ''}
      </form>`);
    document.getElementById('programForm').onsubmit = (e) => {
      e.preventDefault();
      const fd = new FormData(e.target);
      const data = { icon: fd.get('icon'), title: fd.get('title'), shortDesc: fd.get('shortDesc'), fullDesc: fd.get('fullDesc') };
